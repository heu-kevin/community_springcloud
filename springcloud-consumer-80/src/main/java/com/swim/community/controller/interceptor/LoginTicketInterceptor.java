...
import com.swim.community.util.CookieUtil;
...
public class LoginTicketInterceptor implements HandlerInterceptor, CommunityConstant {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (ticket!=null) {
            LoginTicket loginTicket = userService.findLoginTicket(ticket).getBody();
            // check if ticket is expired
            if (loginTicket != null && loginTicket.getStatus() == 0 && loginTicket.getExpired().after(new Date())) {
                // find user
                User user = userService.findUserById(loginTicket.getUserId()).getBody();
