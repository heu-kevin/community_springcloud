    ...
    public String search(String keyword, Page page, Model model) {
        ...
        if (searchResult != null) {
            for (DiscussPost post : searchResult) {
                ...
                map.put("post", post);
                // 作者
                map.put("user", userService.findUserById(post.getUserId()).getBody());
                // 点赞数量
                map.put("likeCount", Long.parseLong(likeService.findEntityLikeCount(ENTITY_TYPE_POST, post.getId()).getBody()));
